...
export function createPlayerController({ scene, config, physicsWorld, terrainHeightAt, mapState, createModel, getCameraYaw, isDown, consumeMousePress }) {
  ...
  function applyWaterPhysics(dt, waterInfo) {
    ...
    applyWaterBuoyancy(playerBody, { mass: config.playerMass, submerged, buoyancy: config.waterBuoyancy, gravityMagnitude: 9.81, dt });
    ...
    applyWaterSpeedDrag(playerBody, { submerged, drag: 2.2, dt });
  ...
  function update(dt) {
    ...
    applyHorizontalSpeed(playerBody, { targetX: targetVX, targetZ: targetVZ, acceleration, braking, mass: config.playerMass, dt });

    if (!inWater && isDown("Space") && grounded && !jumpLatch) {
      const currentVelocity = playerBody.linvel();
      playerBody.setLinvel({ x: currentVelocity.x, y: config.jumpSpeed, z: currentVelocity.z }, true);
      jumpLatch = true;
    }
    ...
    animateHumanoid(playerModel, horizontalSpeed, grounded, running, dt, inWater, config);
  }
  ...
  async function create() {
    ...
    playerModel = await createModel({ shirt: 0x536a78, pants: 0x34383e });
    scene.add(playerModel);
  }

  return { create, update, syncVisual, getWaterInfo, getBody: () => playerBody, getCollider: () => playerCollider, getModel: () => playerModel };
}
